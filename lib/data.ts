...
const loadPublicAlbums = unstable_cache(
  async (): Promise<Album[]> => {
    const supabase = createAdminClient();
    const { data, error } = await supabase
      .from("albums")
  ...
  ["public-albums"],
  { revalidate: 60, tags: ["public-albums"] },
);

const loadAlbumBySlug = unstable_cache(
...
export async function getAlbumBySlug(slug: string): Promise<Album | null> {
  if (!isSupabaseConfigured() || !process.env.SUPABASE_SERVICE_ROLE_KEY)
    return demoAlbums.find((album) => album.slug === slug) ?? null;
  await cookies();
...
export async function getAdminData() {
  if (!isSupabaseConfigured())
    return { albums: demoAlbums, inquiries: [] as Inquiry[] };
