...
export function SiteImageField({
  ...
  const upload = async (file?: File) => {
    ...
    try {
      ...
      const signResponse = await fetch("/api/cloudinary/site-upload", {
        method: "POST",
      });
      ...
      if (!response.ok)
        throw new Error(image.error?.message || "Upload failed.");
  ...
  return (
    <div>
      ...
      <input type="hidden" name={`${name}_public_id`} value={publicId} />
      <div
        className={`relative mt-3 overflow-hidden bg-neutral-100 ${aspect === "hero" ? "aspect-[16/8]" : "aspect-[4/5] max-w-sm"}`}
      >
